...
[작성 규칙]
1. 키워드가 장소라면 해당 장소를 중심으로 이동 동선이 자연스러운 일정을 작성한다.
2. 키워드가 작품/취향/분위기라면 그 키워드가 느껴지는 장소, 음식, 체험을 주제로 일정을 작성한다.
3. 각 일차는 4~6개 코스로 구성한다.
4. 각 코스에는 추천 이유를 한 문장으로 작성한다.
5. 숙소 이동, 식사 장소 추천, 여행의 시작 같은 섹션 제목을 자연스럽게 포함한다.
6. 결과는 반드시 JSON 객체만 반환한다. 마크다운, 설명, 코드블록을 붙이지 않는다.
